...
void VariableGroup::Display( TextContext& Text, float leftMargin, EngineVar* highlightedTweak )
{
    ...
    for (auto iter = m_Children.begin(); iter != m_Children.end(); ++iter)
    {
        ...
        if (iter->second == highlightedTweak)
        {
            ...
            {
                EngineTuning::s_ScrollOffset = std::max(0.0f, EngineTuning::s_ScrollOffset + 0.2f * temp2);
            }
        }
        else
...
void HandleDigitalButtonPress( GameInput::DigitalInput button, float timeDelta, std::function<void ()> action )
{
    ...
    if (Math::Floor(durationHeld * timeStretch) > Math::Floor(oldDuration * timeStretch))
        action();
}
...
void EngineTuning::Display( GraphicsContext& Context, float x, float y, float w, float h )
{
    ...
    float hScale = Graphics::g_DisplayWidth / 1920.0f;
    float vScale = Graphics::g_DisplayHeight / 1080.0f;
    ...
    Text.End();
    Context.SetScissor(0, 0, Graphics::g_DisplayWidth, Graphics::g_DisplayHeight);
}

void EngineTuning::AddToVariableGraph( const std::string& path, EngineVar& var )
{

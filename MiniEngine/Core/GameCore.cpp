...
namespace GameCore
{
    ...
    void RunApplication( IGameApp& app, const wchar_t* className )
    {
        ...
        const LONG winWidth = rc.right - rc.left;
        const LONG winHeight = rc.bottom - rc.top;

        const int x = GetSystemMetrics(SM_CXSCREEN) / 2 - winWidth / 2;
        const int y = GetSystemMetrics(SM_CYSCREEN) / 2 - winHeight / 2;

        g_hWnd = CreateWindow(className, className, WS_OVERLAPPEDWINDOW, x, y,
            winWidth, winHeight, nullptr, nullptr, hInst, nullptr);

...
public:
    ...
    GraphRenderer::GraphHandle AddGraph(PerfGraph* graph)
    {
	    GraphRenderer::GraphHandle ret = (GraphRenderer::GraphHandle)m_Graphs.size();
        m_Graphs.emplace_back(graph);    
        return ret;
    }

    bool Toggle(GraphRenderer::GraphHandle GraphID)
    {
    ...
    Color GetColor(GraphRenderer::GraphHandle GraphID){ return m_Graphs[GraphID]->m_Color;}
...
void GraphRenderer::Initialize( void )
{
    ...
    s_RenderPerfGraphPSO.SetRasterizerState(Graphics::RasterizerDefault);
    s_RenderPerfGraphPSO.SetBlendState(Graphics::BlendTraditional);
    ...
    s_RenderPerfGraphPSO.SetRenderTargetFormats(1, &Graphics::g_OverlayBuffer.GetFormat(), Graphics::g_OverlayBuffer.GetFormat());
    ...
    {    
        InitGraph(GraphRenderer::GraphType::Global );
...
GraphRenderer::GraphHandle GraphRenderer::InitGraph(GraphRenderer::GraphType type)
{
    if (type == GraphRenderer::GraphType::Profile)
        return ProfileGraphs.AddGraph(new PerfGraph(PROFILE_NODE_COUNT, 2));
    else if (type == GraphRenderer::GraphType::Global)
        return GlobalGraphs.AddGraph(new PerfGraph(GLOBAL_NODE_COUNT, 1));
    else
        return PERF_GRAPH_ERROR; 
}

bool GraphRenderer::ManageGraphs( GraphHandle GraphID, GraphRenderer::GraphType Type)
{
...
Color GraphRenderer::GetGraphColor( GraphHandle GraphID, GraphRenderer::GraphType Type)
{
    if (Type == GraphRenderer::GraphType::Profile)
...
void GraphRenderer::Update( XMFLOAT2 InputNode, GraphHandle GraphID, GraphRenderer::GraphType Type)
{
    if (GraphID == PERF_GRAPH_ERROR)
        return;
...
void GraphRenderer::RenderGraphs(GraphicsContext& Context, GraphRenderer::GraphType Type)
{
    if (Type == GraphRenderer::GraphType::Global && GlobalGraphs.Size() == 0 ||
        Type == GraphRenderer::GraphType::Profile && ProfileGraphs.Size() == 0)
    {
    ...
    if (Type == GraphRenderer::GraphType::Profile && ProfileGraphs.GetActiveGraphCount() > 0)
    {
        D3D12_VIEWPORT viewport;
        viewport.TopLeftX = (float)Graphics::g_OverlayBuffer.GetWidth() / 1.3525f;
        viewport.TopLeftY = 0.0f;
        viewport.Width = (float)Graphics::g_OverlayBuffer.GetWidth() / 4.0f;
        viewport.Height = (float)Graphics::g_OverlayBuffer.GetHeight() / (PROFILE_DEBUG_VAR_COUNT + 1);    
        ...
        Context.TransitionResource(Graphics::g_OverlayBuffer, D3D12_RESOURCE_STATE_RENDER_TARGET);
        Context.SetRenderTarget(Graphics::g_OverlayBuffer.GetRTV());
    ...
    {
        D3D12_VIEWPORT viewport;
        viewport.TopLeftX = (float)Graphics::g_OverlayBuffer.GetWidth() / 4.0f;
        viewport.TopLeftY = (float)Graphics::g_OverlayBuffer.GetHeight() / 1.3f;
        viewport.Width = (float)Graphics::g_OverlayBuffer.GetWidth() / 2.0f;
        viewport.Height = (float)Graphics::g_OverlayBuffer.GetHeight() / 8.0f;

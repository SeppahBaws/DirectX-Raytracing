...
namespace EngineProfiling
{
    ...
    {
        if (DrawPerfGraph)
            GraphRenderer::RenderGraphs(Context, GraphRenderer::GraphType::Global );
    }
    ...
    void Display( TextContext& Text, float x, float y, float /*w*/, float /*h*/ )
    {
        ...
        Text.GetCommandContext().SetScissor(0, 0, Graphics::g_DisplayWidth, Graphics::g_DisplayHeight);
    }

} // EngineProfiling

#ifndef RELEASE
ScopedTimer::ScopedTimer(const std::wstring& name): m_Context(nullptr)
{
	EngineProfiling::BeginBlock(name);
}

ScopedTimer::ScopedTimer(const std::wstring& name, CommandContext& Context): m_Context(&Context)
{
...
void StatHistory::RecordStat(uint32_t FrameIndex, float Value)
{
...
float StatHistory::GetLast() const
{
	return m_Recent;
}

float StatHistory::GetMax() const
{
	return m_Maximum;
}

float StatHistory::GetMin() const
{
	return m_Minimum;
}

float StatHistory::GetAvg() const
{
	return m_Average;
}

const float* StatHistory::GetHistory() const
{
	return m_ExtendedHistory;
}

uint32_t StatHistory::GetHistoryLength() const
{
...
uint32_t GpuTimer::GetTimerIndex()
{
	return m_TimerIndex;
}

NestedTimingTree::NestedTimingTree(const std::wstring& name, NestedTimingTree* parent): m_Name(name), m_Parent(parent),
	m_IsExpanded(false), m_IsGraphed(false), m_GraphHandle(PERF_GRAPH_ERROR)
{
}

NestedTimingTree* NestedTimingTree::GetChild(const std::wstring& name)
{
...
NestedTimingTree* NestedTimingTree::NextScope()
{
...
NestedTimingTree* NestedTimingTree::PrevScope()
{
	NestedTimingTree* prev = m_Parent->PrevChild(this);
	return prev == m_Parent ? prev : prev->LastChild();
}

NestedTimingTree* NestedTimingTree::FirstChild()
{
	return m_Children.size() == 0 ? nullptr : m_Children[0];
}

NestedTimingTree* NestedTimingTree::LastChild()
{
	if (!m_IsExpanded || m_Children.size() == 0)
		return this;
...
NestedTimingTree* NestedTimingTree::NextChild(NestedTimingTree* curChild)
{
...
NestedTimingTree* NestedTimingTree::PrevChild(NestedTimingTree* curChild)
{
	ASSERT(curChild->m_Parent == this);
...
void NestedTimingTree::StartTiming(CommandContext* Context)
{
...
void NestedTimingTree::StopTiming(CommandContext* Context)
{
...
void NestedTimingTree::GatherTimes(uint32_t FrameIndex)
{
...
void NestedTimingTree::SumInclusiveTimes(float& cpuTime, float& gpuTime)
{
	cpuTime = 0.0f;
	gpuTime = 0.0f;
	for (auto iter = m_Children.begin(); iter != m_Children.end(); ++iter)
...
void NestedTimingTree::UpdateTimes()
{
	...
	GraphRenderer::Update(XMFLOAT2(TotalCpuTime, TotalGpuTime), 0, GraphRenderer::GraphType::Global);
}

float NestedTimingTree::GetTotalCpuTime()
{
	return s_TotalCpuTime.GetAvg();
}

float NestedTimingTree::GetTotalGpuTime()
{
	return s_TotalGpuTime.GetAvg();
}

float NestedTimingTree::GetFrameDelta()
{
	return s_FrameDelta.GetAvg();
}

void NestedTimingTree::Display(TextContext& Text, float x)
{
...
bool NestedTimingTree::IsGraphed()
{
	return m_IsGraphed;
}

void NestedTimingTree::DeleteChildren()
{
	for (auto node : m_Children)
		delete node;
	m_Children.clear();
...
void NestedTimingTree::DisplayNode( TextContext& Text, float leftMargin, float indent )
{
    ...
    else
    {
        ...
        if (IsGraphed())
        {
            Text.SetColor(GraphRenderer::GetGraphColor(m_GraphHandle, GraphRenderer::GraphType::Profile));
            Text.DrawString("  []\n");
        }
        else
            Text.DrawString("\n");
...
{
    if (m_GraphHandle != PERF_GRAPH_ERROR)
        GraphRenderer::Update( XMFLOAT2(m_CpuTime.GetLast(), m_GpuTime.GetLast()), m_GraphHandle, GraphRenderer::GraphType::Profile);

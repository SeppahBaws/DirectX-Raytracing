...
void GetMeshData(in RayTraceMeshInfo meshInfo, in uint primitiveIdx, in float2 barycentrics, in float2 threadID,
	out float2 uv, out float2 ddxUV, out float2 ddyUV, out float3 normal, out float3 tangent, out float3 bitangent)
{
	...
	// Get vertex tangents
	const float3 tan0 = asfloat(g_Attributes.Load3(meshInfo.m_TangentAttributeOffsetBytes + ii.x * meshInfo.m_AttributeStrideBytes));
	const float3 tan1 = asfloat(g_Attributes.Load3(meshInfo.m_TangentAttributeOffsetBytes + ii.x * meshInfo.m_AttributeStrideBytes));
	const float3 tan2 = asfloat(g_Attributes.Load3(meshInfo.m_TangentAttributeOffsetBytes + ii.x * meshInfo.m_AttributeStrideBytes));
	tangent = normalize(tan0 * bary.x + tan1 * bary.y + tan2 * bary.z);

	// Get vertex bitangents
	const float3 bitan0 = asfloat(g_Attributes.Load3(meshInfo.m_BitangentAttributeOffsetBytes + ii.x * meshInfo.m_AttributeStrideBytes));
	const float3 bitan1 = asfloat(g_Attributes.Load3(meshInfo.m_BitangentAttributeOffsetBytes + ii.x * meshInfo.m_AttributeStrideBytes));
	const float3 bitan2 = asfloat(g_Attributes.Load3(meshInfo.m_BitangentAttributeOffsetBytes + ii.x * meshInfo.m_AttributeStrideBytes));
	bitangent = normalize(bitan0 * bary.x + bitan1 * bary.y + bitan2 * bary.z);
